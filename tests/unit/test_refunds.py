...
class TestRefunds:
    def test_create(self, client):
        with respx.mock:
            respx.post(f"{BASE}/refund").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {"id": 1}})
            )
    ...
    def test_create_with_amount(self, client):
        with respx.mock:
            route = respx.post(f"{BASE}/refund").mock(
    ...
    def test_fetch(self, client):
        with respx.mock:
            respx.get(f"{BASE}/refund/1").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {"id": 1}})
            )
