...
class TestTransferRecipients:
    def test_create(self, client):
        with respx.mock:
            respx.post(f"{BASE}/transferrecipient").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"recipient_code": "RCP_abc"}}
                )
            )
            result = client.transfer_recipients.create(
                type="nuban",
                name="John Doe",
    ...
    def test_list(self, client):
        with respx.mock:
            respx.get(f"{BASE}/transferrecipient").mock(
    ...
    def test_fetch(self, client):
        with respx.mock:
            respx.get(f"{BASE}/transferrecipient/RCP_abc").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"recipient_code": "RCP_abc"}}
                )
            )
    ...
    def test_update(self, client):
        with respx.mock:
            respx.put(f"{BASE}/transferrecipient/RCP_abc").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_delete(self, client):
        with respx.mock:
            respx.delete(f"{BASE}/transferrecipient/RCP_abc").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "message": "Recipient deleted"}
                )
            )
...
class TestAsyncTransferRecipients:
    async def test_create(self, async_client):
        with respx.mock:
            ...
            result = await async_client.transfer_recipients.create(
                type="nuban",
                name="John Doe",
                account_number="0123456789",
                bank_code="058",
