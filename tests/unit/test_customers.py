...
class TestCustomers:
        ...
        with respx.mock:
            respx.post(f"{BASE}/customer").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"email": "test@example.com"}}
                )
            )
            result = client.customers.create(
                email="test@example.com", first_name="John", last_name="Doe", phone="0201234567"
            )
    ...
    def test_list(self, client):
        with respx.mock:
            respx.get(f"{BASE}/customer").mock(
    ...
    def test_fetch(self, client):
        with respx.mock:
            respx.get(f"{BASE}/customer/CUS_abc123").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"customer_code": "CUS_abc123"}}
                )
            )
    ...
    def test_update(self, client):
        with respx.mock:
            respx.put(f"{BASE}/customer/CUS_abc123").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_update_only_sends_provided_fields(self, client):
        with respx.mock:
            ...
            route = respx.put(f"{BASE}/customer/CUS_abc123").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_whitelist(self, client):
        with respx.mock:
            respx.post(f"{BASE}/customer/set_risk_action").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_deactivate_authorization(self, client):
        with respx.mock:
            respx.post(f"{BASE}/customer/deactivate_authorization").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
...
class TestAsyncCustomers:
    async def test_create(self, async_client):
        with respx.mock:
            ...
            result = await async_client.customers.create(
                email="test@example.com", first_name="John", last_name="Doe", phone="0201234567"
            )
            assert result["status"] is True
