...
class TestPlans:
    def test_create(self, client):
        with respx.mock:
            respx.post(f"{BASE}/plan").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"plan_code": "PLN_abc"}}
                )
            )
            result = client.plans.create(
                name="Monthly", interval="monthly", amount=50.0, currency="GHS"
            )
            assert result["status"] is True

    def test_create_converts_amount_to_subunit(self, client):
        with respx.mock:
            route = respx.post(f"{BASE}/plan").mock(
    ...
    def test_fetch(self, client):
        with respx.mock:
            respx.get(f"{BASE}/plan/PLN_abc").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"plan_code": "PLN_abc"}}
                )
            )
    ...
    def test_update(self, client):
        with respx.mock:
            respx.put(f"{BASE}/plan/PLN_abc").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_update_only_sends_provided_fields(self, client):
        with respx.mock:
            route = respx.put(f"{BASE}/plan/PLN_abc").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
...
class TestAsyncPlans:
    async def test_create(self, async_client):
        with respx.mock:
            ...
            result = await async_client.plans.create(
                name="Monthly", interval="monthly", amount=50.0, currency="NGN"
            )
            assert result["status"] is True
