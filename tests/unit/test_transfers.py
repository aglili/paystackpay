...
class TestTransfers:
        ...
        with respx.mock:
            respx.post(f"{BASE}/transfer").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"transfer_code": "TRF_abc"}}
                )
            )
            result = client.transfers.initiate(
                amount=100.0, recipient="RCP_abc", reason="Test", currency="NGN"
            )
            assert result["status"] is True

    def test_initiate_converts_amount(self, client):
        with respx.mock:
            route = respx.post(f"{BASE}/transfer").mock(
    ...
    def test_finalize(self, client):
        with respx.mock:
            respx.post(f"{BASE}/transfer/finalize_transfer").mock(
                return_value=httpx.Response(200, json={"status": True, "data": {}})
            )
    ...
    def test_fetch(self, client):
        with respx.mock:
            respx.get(f"{BASE}/transfer/TRF_abc").mock(
                return_value=httpx.Response(
                    200, json={"status": True, "data": {"transfer_code": "TRF_abc"}}
                )
            )
    ...
    def test_list(self, client):
        with respx.mock:
            respx.get(f"{BASE}/transfer").mock(
                return_value=httpx.Response(200, json={"status": True, "data": []})
            )
...
class TestAsyncTransfers:
    async def test_initiate(self, async_client):
        with respx.mock:
            ...
            result = await async_client.transfers.initiate(
                amount=100.0, recipient="RCP_abc", reason="Test", currency="NGN"
            )
            assert result["status"] is True
